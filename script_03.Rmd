...
sep_new <-
  read_excel("new_data/class_enrollment_summary_by_term_9-21-2020.xlsx", skip = 3) %>%
  clean_names() %>% 
  filter(! is.na(course_title)) %>%
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
...
+ What factors lead to drops in enrollment - large classes, department, professor?

After we have a model, Temperance suggests we should:

+ Uncertainty on why people dropped a course - did they have too much work (individual problem) or did they not like the class (group problem)?

+ Data is still changing, deadline has passed but not impossible to change courses.
...
sep2 <- sep_new %>%
  inner_join(sep_old) %>%
  mutate(u_grad_diff = u_grad_new - u_grad) %>%
  arrange(u_grad_diff)

top_five <- head(sep2, 5)
bottom_five <- tail(sep2, 5)
